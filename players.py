...
class Players:
    def __init__(self, file="synthetic_players.csv", num=0):
        ...
        encoding = {"GK": 0, "CM": 1, "CAM": 2, "FB": 3, "CB": 4, "ST": 5}
    ...
    def comparism(self):
        ...
        sns.heatmap(
            ...
            fmt=".1f",
            cmap="coolwarm",
            xticklabels=self.euclidean_matrix_pd.columns[
                self.euclidean_matrix_pd.columns != "name"
            ],
    ...
    def similarity(self, num=3):
        try:
            ...
            self.similar_num = pd.DataFrame(
                {
                    "player": self.euclidean_matrix_pd[["name"]].to_numpy().flatten(),
                    "most similar": self.euclidean_matrix_pd[f"most similar {num}"],
                }
            )
    ...
    def similarity_matrix(self, matrix, num=3):  # do a new similarity check
        try:
            ...
            similar3 = pd.DataFrame(
                {
                    "player": matrix[["name"]].to_numpy().flatten(),
                    "most similar": matrix["most similar 3"],
                }
            )
            return similar3
        except:
